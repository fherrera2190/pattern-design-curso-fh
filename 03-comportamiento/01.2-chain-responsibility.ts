...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log(
        `Solicitud de compra de $${amount} aprobada por el supervisor.`
      );
      return;
    }
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log(
        `Solicitud de compra de $${amount} aprobada por el manager.`
      );
      return;
    }
...
class Director extends BaseApprover {
  override approveRequest(amount: number): void {
    console.log(
      `Solicitud de compra de $${amount} aprobada por el director.`
    );
  }
}
